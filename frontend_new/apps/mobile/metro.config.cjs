...
const sharedPackageSource = path.resolve(__dirname, "..", "..", "packages", "shared", "src");

// Watch the real source directory only (not the monorepo root which contains the junction)
config.watchFolders = [...(config.watchFolders || []), sharedRNSource, sharedPackageSource];
...
config.resolver.extraNodeModules = {
  ...
  "@hazard-hero/shared": sharedPackageSource,
};
