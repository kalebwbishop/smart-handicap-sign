...
- Q: How should local provisioning endpoints be authenticated? → A: `/configure` requires a device-specific setup or claim code; `/status` and `/scan` remain unauthenticated.
...
- Q: How should `/configure` handle setup and claim codes? → A: `/configure` accepts exactly one of `claim_id` or `setup_code`, rejects requests with both fields, rejects missing fields, and locks configuration attempts for 5 minutes after 5 failed code validations in the current boot session.
- Q: How reusable are local setup or claim codes? → A: Local setup/claim verifiers remain reusable for future Wi-Fi changes until manufacturing, backend registration data, or service tooling revokes or rotates them; firmware must support revoked/expired verifier metadata when such metadata is provisioned.
...
- Q: What does field factory reset clear? → A: Field reset clears Wi-Fi credentials only. Serial number, device auth token, and setup/claim verifier material remain unless a protected manufacturing/service erase path is used.
- Q: How should firmware version compatibility be handled? → A: Firmware sends `X-Firmware-Version` and `X-Firmware-Config-Version` headers on backend runtime requests. Backend enforces configured minimum supported firmware/config versions, and older firmware that cannot support setup-code `/configure` is unsupported for secure provisioning rather than granted a compatibility bypass.
- Q: What frontend accessibility behavior is required? → A: Setup/claim code inputs and provisioning actions must include accessibility labels, hints, role/state semantics, error announcements, invalid-field focus behavior, and non-color-only status communication.
...
2. **Given** valid Wi-Fi credentials and a valid setup or claim code are submitted during provisioning, **When** the device stores the credentials, **Then** it reconnects as a station and exits provisioning mode after connectivity succeeds.
...
- Installer submits an invalid provisioning payload, omits the required setup or claim code for `/configure`, or cancels setup before connection succeeds.
...
- **FR-006**: Provisioning `/configure` MUST require a valid device-specific setup or claim code before accepting Wi-Fi credentials, while `/status` and `/scan` remain available without secrets during provisioning mode.
- **FR-007**: After valid Wi-Fi credentials and a valid setup or claim code are submitted, the device MUST persist the credentials, reconnect as a station, and leave provisioning mode after connection succeeds.
...
- **FR-017**: `/configure` MUST enforce an authorization matrix covering missing code, both-code input, invalid code, revoked/expired verifier metadata when available, replay or repeated failure, and valid code behavior.
- **FR-018**: The device MUST lock local `/configure` credential writes for 5 minutes after 5 failed setup/claim code validation attempts during the current boot session and MUST return generic failure responses that do not reveal which portion of the secret failed.
...
- **FR-020**: Field reset behavior MUST clear Wi-Fi credentials without clearing serial number, device auth token, or setup/claim verifier material; full identity erasure MUST require a protected manufacturing/service path outside normal installer setup.
...
- **FR-023**: Operational metrics MUST be defined for provisioning failures, setup-code lockouts, status poll failures and recoveries, classify submissions, auth failures, and OTA validation, even if the first implementation records them through structured logs or device events rather than a dedicated metrics backend.
- **FR-024**: Frontend provisioning flows MUST expose setup/claim code fields and errors with accessible labels, hints, role/state semantics, screen-reader error announcements, focus movement to invalid fields, and non-color-only status indicators.
...
- **Authentication**: Classification submissions use a device bearer token. Device status polling uses the existing firmware-facing status contract. Local provisioning is only available while provisioning mode is active; local `/configure` requires a valid device-specific setup or claim code, while `/status` and `/scan` remain unauthenticated and must not return secrets.
...
- **Provisioning/Wi-Fi**: Local provisioning handles missing or failed Wi-Fi credentials, scans available networks, accepts `/configure` only with a valid setup or claim code, stores valid credentials, reconnects as a station, and falls back after retry exhaustion.
...
- **Provisioning Authorization Code**: Device-specific setup or claim code required to submit Wi-Fi credentials to local `/configure` during provisioning mode.
...
- Setup/claim code validation MUST resist brute-force attempts by tracking failures and applying the configured lockout, and MUST avoid response wording that confirms whether a submitted code is close to valid.
- Replay of a setup/claim code is permitted only for Wi-Fi reconfiguration while the local verifier remains active; revoked or expired verifier metadata MUST disable replay when such metadata is provisioned.
- Provisioning responses MUST NOT expose saved Wi-Fi passwords, device bearer tokens, or the stored setup or claim code.
- Provisioning `/configure` MUST reject Wi-Fi credential submissions that omit or provide an invalid device-specific setup or claim code.
...
- Frontend setup/claim code inputs MUST have accessible labels and hints that describe the expected code without exposing secrets.
...
- Firmware and backend logs MUST use structured, secret-safe fields for provisioning attempts, setup-code lockouts, token failures, status polling failures and recoveries, classify submissions, wave detections, certificate failures, and OTA validation.
...
- Metrics definitions MUST include counters or timers for setup-code validation failures, lockouts, status poll failures, status recoveries, classify submissions, auth failures, OTA validation, and provisioning success.
...
- **SC-006**: During provisioning validation, `/configure` rejects credential submissions without a valid setup or claim code and never returns stored Wi-Fi passwords, device bearer tokens, or setup/claim codes.
...
- **SC-009**: During provisioning validation, 5 invalid setup/claim code attempts cause a 5-minute `/configure` lockout, do not save Wi-Fi credentials, and do not reveal secret validation details.
- **SC-010**: During frontend provisioning validation, setup/claim code inputs and errors are reachable and understandable through screen-reader semantics, keyboard/focus behavior where applicable, and non-color-only visual indicators.
- **SC-011**: During runtime validation, backend logs or device event history show backend-observed wave detection, auth failure threshold exceeded, and observed firmware version plus firmware-reported status failure/recovery, OTA validation, and provisioning success without exposing device tokens, setup codes, hashes, salts, or Wi-Fi passwords.
